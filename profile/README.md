...
| [**compliance-aiops**](https://github.com/AIops-tools/Compliance-AIops) | **Meta-tool** — turns the audit trails these tools already write into framework-mapped (HIPAA/PCI-DSS/SOC 2/GDPR), hash-chain-sealed evidence bundles, exportable as NIST OSCAL 1.2.3 assessment results | `pip install compliance-aiops` | 19 |
| [**ai-guardian**](https://github.com/AIops-tools/AI-Guardian) | On-endpoint local-LLM (Ollama) observability + governance — model allow/deny policy, provenance drift, a deterministic secrets/PII/jailbreak prompt scanner, route-through guarding, and a transparent capture proxy for clients that did not opt in | `pip install ai-guardian-aiops` | 21 |
...
| [**olvm-aiops**](https://github.com/AIops-tools/OLVM-AIops) | Oracle Linux Virtualization Manager / oVirt 4.5 — data centers, clusters, hosts, storage domains, VMs, events, jobs; engine, host, storage & VM health RCA (certificate and backup alerts, recovery-aware); read-only | `pip install olvm-aiops` | 17 |
...
| [**minio-aiops**](https://github.com/AIops-tools/MinIO-AIops) | MinIO object storage — bucket / capacity / replication health, policy & lifecycle visibility, object-lock (WORM) retention governance, IAM, guarded bucket writes | `pip install minio-aiops` | 48 |
