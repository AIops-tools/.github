...
| [**fabric-aiops**](https://github.com/AIops-tools/Fabric-AIops) | Network **controllers** — Cisco Meraki, Cisco Catalyst Center, Arista CloudVision, Ubiquiti UniFi (org→network→device); uplink loss/latency RCA, fleet health score, config-template drift, guarded remediation | `pip install fabric-aiops` | 32 |
...
| [**monitoring-aiops**](https://github.com/AIops-tools/Monitoring-AIops) | SolarWinds Orion (SWIS/SWQL) + Paessler PRTG + Zabbix — canned SWQL, alert dedup/rollup, health, maintenance windows | `pip install monitoring-aiops` | 40 |
...
| [**mysql-aiops**](https://github.com/AIops-tools/MySQL-AIops) | MySQL + MariaDB DBA-ops — slow-query / replication-lag / blocking-lock RCA, index & table maintenance, guarded remediation | `pip install mysql-aiops` | 33 |
| [**proxy-aiops**](https://github.com/AIops-tools/Proxy-AIops) | Reverse proxies — Traefik / Caddy / HAProxy: route & backend health RCA, certificate visibility, guarded config writes | `pip install proxy-aiops` | 26 |
| [**queue-aiops**](https://github.com/AIops-tools/Queue-AIops) | Middleware — Redis + RabbitMQ: queue-depth / consumer-lag / memory-pressure RCA, guarded queue & key operations | `pip install queue-aiops` | 26 |
| [**minio-aiops**](https://github.com/AIops-tools/MinIO-AIops) | MinIO object storage — bucket / capacity / replication health, policy & lifecycle visibility, guarded bucket writes | `pip install minio-aiops` | 29 |
| [**xcpng-aiops**](https://github.com/AIops-tools/XCPng-AIops) | XCP-ng via Xen Orchestra — VMs, snapshots, hosts, pools, storage repositories, guarded lifecycle writes | `pip install xcpng-aiops` | 27 |
| [**identity-aiops**](https://github.com/AIops-tools/Identity-AIops) | SSO / IAM — Keycloak + Authentik: realm / client / user health, login-failure analysis, guarded identity writes | `pip install identity-aiops` | 27 |
| [**cicd-aiops**](https://github.com/AIops-tools/CICD-AIops) | Self-managed CI — GitLab + Gitea: pipeline-failure RCA, runner health, job & artifact hygiene, guarded pipeline operations | `pip install cicd-aiops` | 26 |
